...
//更新所有状态的位姿
	...
	onScanmatchUpdate();//更新scanmatch的 hypotheses，weightSums，m_neff等。获得最优粒子
	
	updateTreeWeights(false);//采样之前权重更新一次
	...
	if (m_outputStream.is_open()){
 	...
 	resample(plainReading, adaptParticles, reading_copy);//重采样 原理：粒子集对目标分布的近似越差，则权重的方差越大。
														 //可以用Neff作为权重值离差的量度
      } else {
	...
	for (ParticleVector::iterator it=m_particles.begin(); it!=m_particles.end(); it++){	
      ...
      updateTreeWeights(false);//采样之后更新一次权重
      //		cerr  << ".done!" <<endl;
